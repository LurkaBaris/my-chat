...
Доставка сообщений: server actions (этап 2) → SSE / EventSource (этап 3) → UX (этап 4)
```
...
### Задача 4.1 — Время в ленте и в превью бесед

**Что сделать:**

- У каждого сообщения в открытом чате показывать время отправки (и дату, если сообщение не за сегодня).
- В сайдбаре для превью: сегодня — `HH:MM`, вчера/раньше — короткая дата (например `24.07`).
- Форматирование вынести в одну утилиту (уже есть задел в `formatConversationTime` — расширить и переиспользовать).

**Критерии приёмки:**

- [ ] В ленте у сообщений видно время
- [ ] Старые сообщения/беседы не выглядят как «сегодняшние» только с часами
- [ ] Один и тот же хелпер (или семейство) используется для списка и ленты

---

### Задача 4.2 — Пустые состояния и базовая мобильная вёрстка

**Что сделать:**

- Empty-state открытого чата без выбранной беседы уже есть — добить: пустая лента («Напишите первое сообщение») и отсутствие бесед в сайдбаре (если ещё не идеально).
- На узком экране: сайдбар и лента не ломают layout (хотя бы: на мобиле список OR чат, без обязательного resize-handle).
- Убрать из UI элементы без функциональности (фейковый «Онлайн», нерабочие вложения) — либо реальное поведение, либо не показывать.

**Критерии приёмки:**

- [ ] Понятно, что делать, если чатов нет / лента пустая / беседа не выбрана
- [ ] На ширине ~375px можно открыть список и конкретный чат без горизонтального скролла всей страницы
- [ ] В шапке/композере нет контролов, которые ничего не делают

---

### Задача 4.3 — Логин вместо email

Сейчас идентификатор пользователя — email; для учебного чата это выглядит чужеродно. Перевести весь пользовательский флоу на **логин** (username).

**Что сделать:**

- В схеме `User`: уникальное поле `login` (строка); email убрать из UI. В БД email можно сделать опциональным (`String?`) под Prisma-адаптер Auth.js либо убрать, если адаптеру не мешает — зафиксировать выбор в миграции.
- Валидация Zod: логин (например trim, lowercase, `min`/`max`, только латиница/цифры/`_`), уникальность при регистрации.
- Регистрация и логин: формы и Credentials `authorize` ищут пользователя по `login`, не по email.
- Старт диалога 1-на-1: ввод **логина** собеседника вместо email (`startConversationByLogin`).
- Отображение: в сайдбаре профиля и в заголовке чата — `name` или `login` (email нигде в UI не торчит).
- Обновить тексты ошибок/плейсхолдеров и README (как регистрироваться и кого искать для чата).

**Критерии приёмки:**

- [ ] Регистрация и вход только по логину + паролю
- [ ] Повторный логин отклоняется; несуществующий логин при старте чата — нейтральная ошибка (как сейчас с email)
- [ ] Новый диалог создаётся по логину собеседника
- [ ] В UI нет полей/подписей email в auth и создании чата
- [ ] Миграция применена; старые тестовые данные либо смигрированы, либо БД пересоздана локально осознанно

---

## Definition of Done — проект целиком

- [ ] Два пользователя (вкладка + инкогнито) переписываются в одном диалоге без F5
- [ ] Список бесед, создание 1-на-1, отправка/чтение, SSE — работают end-to-end
- [ ] Время в ленте и превью (4.1); пустые состояния и мобильный каркас (4.2); вход/поиск по логину (4.3)
- [ ] Пароли только как bcrypt; `passwordHash` не в ответах/логах/SSE
- [ ] Неавторизованный не читает чужие stream и страницы чата
- [ ] `npm run check` + `npm run build` зелёные
- [ ] README: стек, локальный запуск, Prisma, Auth.js vs свой код, кратко про SSE
